...
class Administrativos extends CI_Controller { 
	...
	public function __construct(){ 
		...
		$this->load->model('model_administrativos', 'model_administrativos');
		...
		$this->data_sistema['permiso']   = $this->modelo->permiso_usuario_actual(5);		
	...
	//catalogo de usuarios
	  public function index(){
	   if ( $this->session->userdata('session') !== TRUE ) {
          ...
          $data=array();
          $this->data_sistema['subcategorias']   = $this->model_administrativos->listado_subcategorias();
          //print_r($this->data_sistema['subcategorias']); die;
          $data = array_merge($this->data_sistema,$data);
	      switch ($this->session->userdata('id_perfil')) {    
		        case 1: 
		            $this->load->view( 'administrativos/administrativos', $data);
		          break;
		        case 2:
	...
	public function detalle($url_hijo, $id_hijo,$num_hijo,   $id_encabezado='MA==', $id_seleccion='MA=='){
	   ...
	   if ( $this->session->userdata('session') !== TRUE ) {
          ...
          $data['subcategorias']   = $this->model_administrativos->listado_subcategorias();
		  ...
		  $data['sub_pregunta']   = $this->model_administrativos->listado_sub_pregunta($data);	  

		  //print_r($data['sub_pregunta']); die;

		  $data['id_principal']			 =  5 ;
		  $data['selectores']   = $this->model_administrativos->listado_selectores_marcados($data);	  
	      ...
	      switch ($this->session->userdata('id_perfil')) {    
		        case 1: 
		            $this->load->view( 'administrativos/detalle', $data);
		          break;
		        case 2:
		        case 3:  //funcionario y admin tambien pueden
		             $this->load->view( 'administrativos/detalle', $data);
...
//este procedimiento va a servir tanto para profesionales como administrativos, guardar sus selectores
	public function guardar_selectores_marcados() {
      ...
      $data['url_principal'] = $this->input->post('url_principal');
          ...
          $this->model_administrativos->guardar_selectores_marcados($data);
