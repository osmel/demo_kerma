...
class Incrementos extends CI_Controller {

	public function __construct(){ 
		parent::__construct();
		$this->load->model('admin/modelo', 'modelo'); 
		$this->load->model('admin/modelo_arbol', 'modelo_arbol'); 
		$this->load->model('model_incrementos', 'model_incrementos');
	...
	//catalogo de usuarios
	  public function index(){
	   if ( $this->session->userdata('session') !== TRUE ) {
          ...
          $data=array();
          $this->data_sistema['subcategorias']   = $this->model_incrementos->listado_subcategorias();
          $data = array_merge($this->data_sistema,$data);
	      switch ($this->session->userdata('id_perfil')) {    
		        case 1: 
		            $this->load->view( 'incrementos/incrementos', $data);
		          break;
		        case 2:
	...
	public function detalle($encabezado, $id_encabezado,$id_seccion='MA=='){
	   if ( $this->session->userdata('session') !== TRUE ) {
		  ...
		  $data['sub_pregunta']   = $this->model_incrementos->listado_sub_pregunta($data);	          

          $data['subcategorias']   = $this->model_incrementos->listado_subcategorias();
          $data = array_merge($this->data_sistema,$data);
	      switch ($this->session->userdata('id_perfil')) {    
		        case 1: 
		            $this->load->view( 'incrementos/detalle', $data);
		          break;
		        case 2:
		        case 3:  //funcionario y admin tambien pueden
		             $this->load->view( 'incrementos/detalle', $data);
