...
class Informacion_general extends CI_Controller {
	...
	public function __construct(){ 
		...
		//print_r( json_encode( self::menu_nodo(1) ) );		die;
		...
		//print_r( ($this->data_sistema['permiso']) ); die;	
	  ...
	  public function index(){
	   if ( $this->session->userdata('session') !== TRUE ) {
	      ...
	      switch ($this->session->userdata('id_perfil')) {    
		        case 1: 
		            $this->load->view( 'informacion_general/informacion_general', $data);
		          break;
		        case 2:
	...
	public function leer_datos_general(){
		  $data=$_POST;

		  $data['datos']  = $this->model_informacion_general->leer_datos($data);	  
	...
	public function actualizar_datos_general(){
		  $data=$_POST;

		  $data['datos']  = $this->model_informacion_general->actualizar_datos($data);	  
		  echo json_encode($data);
	...
	public function detalle($encabezado, $id_encabezado,$id_seccion='MA==' ){
	   if ( $this->session->userdata('session') !== TRUE ) {
		  ...
		  $data['sub_pregunta']   = $this->model_informacion_general->listado_sub_pregunta($data);	  
		  //print_r($data['sub_pregunta']); die;
          $data['subcategorias']   = $this->model_informacion_general->listado_subcategorias();
	      ...
	      switch ($this->session->userdata('id_perfil')) {    
		        case 1: 
		            $this->load->view( 'informacion_general/detalle', $data);
		          break;
		        case 2:
		        case 3:  //funcionario y admin tambien pueden
		             $this->load->view( 'informacion_general/detalle', $data);
