...
class Operacion_abogados extends CI_Controller {

	public function __construct(){ 
		parent::__construct();
		$this->load->model('admin/modelo', 'modelo'); 
		$this->load->model('admin/modelo_arbol', 'modelo_arbol'); 
		$this->load->model('model_operacion_abogados', 'model_operacion_abogados');
		...
		$this->data_sistema['permiso']   = $this->modelo->permiso_usuario_actual(4);
	...
	//catalogo de usuarios
	  public function index(){
	   if ( $this->session->userdata('session') !== TRUE ) {
          ...
          $data=array();
          $this->data_sistema['subcategorias']   = $this->model_operacion_abogados->listado_subcategorias();
          $data = array_merge($this->data_sistema,$data);
	      switch ($this->session->userdata('id_perfil')) {    
		        case 1: 
		            $this->load->view( 'operacion_abogados/operacion_abogados', $data);
		          break;
		        case 2:
	...
	public function detalle($encabezado, $id_encabezado,$id_seccion='MA=='){
	   if ( $this->session->userdata('session') !== TRUE ) {
	      ...
	      switch ($this->session->userdata('id_perfil')) {    
		        case 1: 
		            $this->load->view( 'operacion_abogados/detalle', $data);
		          break;
		        case 2:
		        case 3:  //funcionario y admin tambien pueden
		             $this->load->view( 'operacion_abogados/detalle', $data);
