...
class Otras_prestaciones extends CI_Controller {

	public function __construct(){ 
		parent::__construct();
		$this->load->model('admin/modelo', 'modelo'); 
		$this->load->model('admin/modelo_arbol', 'modelo_arbol'); 
		$this->load->model('model_otras_prestaciones', 'model_otras_prestaciones');
		...
		$this->data_sistema['permiso']   = $this->modelo->permiso_usuario_actual(12);
	...
	//catalogo de usuarios
	  public function index(){
	   if ( $this->session->userdata('session') !== TRUE ) {
          ...
          $data=array();
          $this->data_sistema['subcategorias']   = $this->model_otras_prestaciones->listado_subcategorias();
          $data = array_merge($this->data_sistema,$data);
	      switch ($this->session->userdata('id_perfil')) {    
		        case 1: 
		            $this->load->view( 'otras_prestaciones/otras_prestaciones', $data);
		          break;
		        case 2:
	...
	public function detalle($encabezado, $id_encabezado,$id_boton,$id_seccion='MA=='){ //Array ( [encabezado] => estudios12 [id_encabezado] => 47 [id_seccion] => 1 )
	   if ( $this->session->userdata('session') !== TRUE ) {
	        redirect('login');
	    } else {
          $data=array();
          ...
          $data['id_boton'] = base64_decode($id_boton);
          $data['id_seccion'] = base64_decode($id_seccion);

          $data['botones']   = $this->model_otras_prestaciones->listado_botones($data);	          
   	  	  $btnes = explode('|', ($data['botones'][0]->grupos_id) );
  	      $data['id_boton']=  (( $data['id_boton'] == 0)  && ($btnes[0]!='')) ? $btnes[0] : $data['id_boton'];

		  $data['sub_pregunta']   = $this->model_otras_prestaciones->listado_sub_pregunta($data);	      
          $data['subcategorias']   = $this->model_otras_prestaciones->listado_subcategorias();

          //print_r($data['sub_pregunta']); die;
          $data = array_merge($this->data_sistema,$data);
	      switch ($this->session->userdata('id_perfil')) {    
		        case 1: 
		            $this->load->view( 'otras_prestaciones/detalle', $data);
		          break;
		        case 2:
		        case 3:  //funcionario y admin tambien pueden
		             $this->load->view( 'otras_prestaciones/detalle', $data);
