...
class Prestaciones extends CI_Controller {

	public function __construct(){ 
		parent::__construct();
		$this->load->model('admin/modelo', 'modelo'); 
		$this->load->model('admin/modelo_arbol', 'modelo_arbol'); 
		$this->load->model('model_prestaciones', 'model_prestaciones');
		...
		$this->data_sistema['permiso']   = $this->modelo->permiso_usuario_actual(11);		
	...
	//catalogo de usuarios
	  public function index(){
	   if ( $this->session->userdata('session') !== TRUE ) {
          ...
          $data=array();
          $this->data_sistema['subcategorias']   = $this->model_prestaciones->listado_subcategorias();
          $data = array_merge($this->data_sistema,$data);
	      switch ($this->session->userdata('id_perfil')) {    
		        case 1: 
		            $this->load->view( 'prestaciones/prestaciones', $data);
		          break;
		        case 2:
	...
	public function detalle($encabezado, $id_encabezado,$id_boton,$id_seccion='MA=='){
	   if ( $this->session->userdata('session') !== TRUE ) {
	        redirect('login');
	    } else {
          $data=array();
          ...
          $data['botones']   = $this->model_prestaciones->listado_botones($data);	          
   	  	  $btnes = explode('|', ($data['botones'][0]->grupos_id) );
  	      $data['id_boton']=  (( $data['id_boton'] == 0)  && ($btnes[0]!='')) ? $btnes[0] : $data['id_boton'];

		  $data['sub_pregunta']   = $this->model_prestaciones->listado_sub_pregunta($data);	             
          $data['subcategorias']   = $this->model_prestaciones->listado_subcategorias();
	      ...
	      switch ($this->session->userdata('id_perfil')) {    
		        case 1: 
		            $this->load->view( 'prestaciones/detalle', $data);
		          break;
		        case 2:
		        case 3:  //funcionario y admin tambien pueden
		             $this->load->view( 'prestaciones/detalle', $data);
