          ...
          $result = $this->db->query("
            ...
            select id_principal,  principal,  url_principal,
                        ...
                        GROUP_CONCAT(DISTINCT(url_hijo) ORDER BY id_hijo SEPARATOR '|') url_hijos,
                         ...
                         (case when (pestanas=0) then grupos else encabezados end) botones, 
                         (case when (pestanas=0) then id_grupos else id_encabezados end) id_botones
                ...
                (SELECT m.id id_principal,  m.nombre principal,  m.url url_principal,m.icono,
                        ...
                        ,0 pestanas,GROUP_CONCAT(DISTINCT(e.nombre) ORDER BY e.id SEPARATOR '|') encabezados, GROUP_CONCAT(DISTINCT(e.id) ORDER BY e.id SEPARATOR '|') id_encabezados, GROUP_CONCAT(DISTINCT(gch.descripcion) ORDER BY gch.id SEPARATOR '|') grupos, GROUP_CONCAT(DISTINCT(gch.id) ORDER BY gch.id SEPARATOR '|') id_grupos
                ...
                SELECT  
                            ...
                            ,1 pestanas,GROUP_CONCAT(DISTINCT(e.nombre) ORDER BY e.id SEPARATOR '|') encabezados, GROUP_CONCAT(DISTINCT(e.id) ORDER BY e.id SEPARATOR '|') id_encabezados, GROUP_CONCAT(DISTINCT(gch.descripcion) ORDER BY gch.id SEPARATOR '|') grupos, GROUP_CONCAT(DISTINCT(gch.id) ORDER BY gch.id SEPARATOR '|') id_grupos
                FROM kerma_cat_view_modulos m
 				INNER JOIN kerma_cat_view_modulo__encabezado_pregunta r ON r.id_modulo = m.id
                INNER JOIN kerma_cat_view_encabezado_pregunta e ON r.id_encabezado_pregunta = e.id
                  INNER JOIN kerma_cat_view_preguntas p ON r.id = p.id_modulo__encabezado_pregunta
