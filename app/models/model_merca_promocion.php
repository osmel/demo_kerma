<?php if(! defined('BASEPATH')) exit('No tienes permiso para acceder a este archivo');
  ...
  class model_merca_promocion extends CI_Model{
...
/*
SELECT e.id, e.nombre, m.nombre principal,m.url url_principal,e.url url_hijo, r.id id_encabezado
...
SELECT 
CASE
    WHEN (p.wildcard IS NULL)  THEN p.nombre
    WHEN (p.wildcard IS NOT NULL)  THEN p.wildcard
    ELSE "aaa"
END as pregunta,

e.id, e.nombre, m.nombre principal, p.nombre pregunta_encabezado, r.id, p.id_numeracion_pregunta, p.identificador, p.wildcard,
t.nombre tipo_pregunta,
vp.descripcion valor_predefinido,
vp.id id_valor_predefinido,
tvp.descripcion tipo_valor_predefinido,
