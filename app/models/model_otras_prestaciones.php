<?php if(! defined('BASEPATH')) exit('No tienes permiso para acceder a este archivo');
  ...
  class model_otras_prestaciones extends CI_Model{
...
FROM kerma_cat_view_modulos m
  ...
  WHERE m.id =12 AND r.id =49 and cha.id=108
...
SELECT  e.id, e.nombre, m.nombre principal, p.nombre pregunta, r.id, p.id_numeracion_pregunta, p.identificador, p.wildcard,
    ...
    t.id id_tipo_pregunta,
    t.nombre tipo_pregunta,
    GROUP_CONCAT(DISTINCT(vp.descripcion) ORDER BY vp.id SEPARATOR '|') valores, 
    GROUP_CONCAT(DISTINCT(vp.id) ORDER BY vp.id SEPARATOR '|') valores_id,
    tvp.id id_tipo_valores_predefinidos,tvp.descripcion
...
Planes de jubilación: 49

Planes de jubilaciÃ³n : 108
Edad para el retiro   : 109
Sistema de sucesiÃ³n   :110
108|109|110
            ...
            $this->db->join($this->capital_humano_actual_por_preguntas.' chp', 'p.id = chp.id_pregunta');
            $this->db->join($this->capital_humano_actual.' cha', 'cha.id = chp.id_capital_humano_actual');
              $this->db->join($this->grupo_capital_humano.' gch', 'gch.id = cha.id_grupo_capital_humano');
              $this->db->join($this->categoria_capital_humano.' cch', 'cch.id = cha.id_categoria_capital_humano');
                $this->db->join($this->tipo_capital_humano.' tch', 'tch.id = cch.id_tipo_capital_humano');
...
*/
      ...
      public function listado_sub_pregunta($data){
            ...
            //$this->db->select( 'concat("{",GROUP_CONCAT(DISTINCT(concat("''",gch.descripcion,"'':",cha.id )) ORDER BY cha.id SEPARATOR ","),"}") as grupos',false);
