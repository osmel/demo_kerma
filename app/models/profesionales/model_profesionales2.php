...
class model_profesionales2 extends CI_Model{
    ...
    function __construct(){
        ...
        $this->subdetalle_tipo_profesional         =      $this->db->dbprefix('cat_subdetalle_tipo_profesional');
    ...
    */
    ...
    public function listado_niveles()
    {
        ...
        $this->db->from($this->subdetalle_tipo_profesional);
        $this->db->where('id_detalle_tipo_profesional',2);
        $result = $this->db->get();
        if ($result->num_rows() > 0) {
            return $result->result();
        }else {
